/**
 * KlareErfassungDialog — pre-generated create/edit dialog for KlareErfassung.
 *
 ...
 * KlareErfassungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<KlareErfassung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<KlareErfassungDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { KlareErfassung, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/KlareErfassung';
...
/** Widened prefill type for KlareErfassungDialog.defaultValues — see file header. */
export type KlareErfassungDialogDefaults = Omit<KlareErfassung['fields'], 'kategorie' | 'status'> & {
    kategorie?: LookupValue | string;
    status?: LookupValue | string;
  };

interface KlareErfassungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: KlareErfassung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: KlareErfassungDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['klare_erfassung']?.['kategorie'] ?? [],
  status: LOOKUP_OPTIONS['klare_erfassung']?.['status'] ?? [],
};
...
export function KlareErfassungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KlareErfassungDialogProps) {
  const [fields, setFields] = useState<Partial<KlareErfassung['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<KlareErfassung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'klare_erfassung');
      await onSubmit(clean as KlareErfassung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<KlareErfassung['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Klare Erfassung bearbeiten' : 'Klare Erfassung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'titel': (
      <div key="titel" className="space-y-1.5">
        ...
        <Input
          id="titel"
          placeholder="z. B. Projektplan erstellen"
          value={fields.titel ?? ''}
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Was ist zu tun? Kontext, Ziel, Anforderungen..."
      ...
      <div key="kategorie" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'allgemein'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'allgemein' ? undefined : 'allgemein') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'allgemein'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'wichtig'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'wichtig' ? undefined : 'wichtig') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'wichtig'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'dringend'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'dringend' ? undefined : 'dringend') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'dringend'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'optional'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'optional' ? undefined : 'optional') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'optional'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="anmerkungen"
          placeholder="Besonderheiten, Kontakte, Links..."
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "beschreibung": "Beschreibung", "kategorie": "Kategorie", "datum": "Datum", "status": "Status", "anmerkungen": "Anmerkungen", "anhang": "Anhang"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KLARE_ERFASSUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
