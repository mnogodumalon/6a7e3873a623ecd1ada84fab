...
import type { KlareErfassung, CreateKlareErfassung } from '@/types/app';
...
export class LivingAppsService {
  // --- KLARE_ERFASSUNG ---
  static async getKlareErfassung(): Promise<KlareErfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KLARE_ERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as KlareErfassung[];
    return enrichLookupFields(records, 'klare_erfassung');
  }
  static async getKlareErfassungEntry(id: string): Promise<KlareErfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KLARE_ERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as KlareErfassung;
    return enrichLookupFields([record], 'klare_erfassung')[0];
  }
  static async createKlareErfassungEntry(fields: CreateKlareErfassung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.KLARE_ERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'klare_erfassung') });
    return { ...data, record_id: data.id };
  }
  static async updateKlareErfassungEntry(id: string, fields: Partial<CreateKlareErfassung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.KLARE_ERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'klare_erfassung') });
    return { ...data, record_id: data.id };
  }
  static async deleteKlareErfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KLARE_ERFASSUNG}/records/${id}`);
  }
