...
import { KlareErfassungViewDialog } from '@/components/dialogs/KlareErfassungViewDialog';
...
const KLAREERFASSUNG_FIELDS = [
  { key: 'titel', label: 'Titel', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'allgemein', label: 'Allgemein' }, { key: 'wichtig', label: 'Wichtig' }, { key: 'dringend', label: 'Dringend' }, { key: 'optional', label: 'Optional' }] },
...
const ENTITY_TABS = [
  { key: 'klare_erfassung', label: 'Klare Erfassung', pascal: 'KlareErfassung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('klare_erfassung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'klare_erfassung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'klare_erfassung': return (data as any).klareErfassung as KlareErfassung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'klare_erfassung': return KLAREERFASSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'klare_erfassung': return {
        create: (fields: any) => LivingAppsService.createKlareErfassungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKlareErfassungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKlareErfassungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'klare_erfassung' || dialogState?.entity === 'klare_erfassung') && (
        <KlareErfassungDialog
          open={createEntity === 'klare_erfassung' || dialogState?.entity === 'klare_erfassung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'klare_erfassung' ? handleUpdate : (fields: any) => handleCreate('klare_erfassung', fields)}
          defaultValues={dialogState?.entity === 'klare_erfassung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['KlareErfassung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['KlareErfassung']}
        />
      )}
      {viewState?.entity === 'klare_erfassung' && (
        <KlareErfassungViewDialog
          open={viewState?.entity === 'klare_erfassung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'klare_erfassung', record: r }); }}
        />
      )}
